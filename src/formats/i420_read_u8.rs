...
pub(crate) fn read_i420<Vis: I420Visitor>(
    ...
    if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
        unsafe {
            return read_i420_avx2(src_width, src_height, src, window, visitor);
        }
    }
...
#[inline(never)]
unsafe fn read_i420_avx2<Vis: I420Visitor>(
    src_width: usize,
    src_height: usize,
    src: &[u8],
    window: Option<Rect>,
...
{
    // Load Y pixels
    let y00 = V::load_u8(y_ptr.add(y00_offset));
    let y01 = V::load_u8(y_ptr.add(y00_offset + V::LEN));
    let y10 = V::load_u8(y_ptr.add(y10_offset));
    let y11 = V::load_u8(y_ptr.add(y10_offset + V::LEN));

    // Load U and V
    let u = V::load_u8(u_ptr.add(uv_offset));
    let v = V::load_u8(v_ptr.add(uv_offset));
