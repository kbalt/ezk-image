...
pub(crate) struct Read2Plane<'a, P: Primitive> {
    y: &'a [u8],
    uv: &'a [u8],
...
impl<'a, P: Primitive> Read2Plane<'a, P> {
...
impl<P: Primitive> Yuv420Src for Read2Plane<'_, P> {
    #[inline(always)]
    unsafe fn read<V: Vector>(&mut self, x: usize, y: usize) -> Yuv420Block<V> {
        ...
        // Load U and V
        let uv0 = P::load::<V>(&self.uv[uv_offset..]);
        let uv1 = P::load::<V>(&self.uv[uv_offset + V::LEN * P::SIZE..]);
