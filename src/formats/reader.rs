    ...
    R: ImageReader,
{
    ...
    if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
        ...
        unsafe fn call<R>(src_width: usize, src_height: usize, window: Rect, reader: R)
        where
            R: ImageReader,
        {
            read_impl::<__m256, _>(src_width, src_height, window, reader);
        }

        // Safety: Did a feature check
        unsafe {
            return call::<R>(src_width, src_height, window, reader);
        }
    }
    ...
    unsafe { read_impl::<f32, _>(src_width, src_height, window, reader) }
}

#[inline(always)]
unsafe fn read_impl<V: Vector, R: ImageReader>(
    width: usize,
    height: usize,
    window: Rect,
    mut reader: R,
