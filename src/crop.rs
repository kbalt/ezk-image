...
impl<T: ImageRef> ImageRef for Cropped<T> {
    ...
    fn planes(&self) -> impl Iterator<Item = (&[u8], usize)> {
        ...
        match self.format() {
            I420 => crop_planes(I420_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            I422 => crop_planes(I422_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            I444 => crop_planes(I444_PLANES, read_planes(self.0.planes()).unwrap(), self.1),

            I010 | I012 => crop_planes(I01X_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            I210 | I212 => crop_planes(I21X_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            I410 | I412 => crop_planes(I41X_PLANES, read_planes(self.0.planes()).unwrap(), self.1),

            NV12 => crop_planes(NV12_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            YUYV => crop_planes(YUYV_PLANES, read_planes(self.0.planes()).unwrap(), self.1),

            RGBA | BGRA => crop_planes(RGBA_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
            RGB | BGR => crop_planes(RGB_PLANES, read_planes(self.0.planes()).unwrap(), self.1),
        }
    }
...
impl<T: ImageMut> ImageMut for Cropped<T> {
    fn planes_mut(&mut self) -> impl Iterator<Item = (&mut [u8], usize)> {
        ...
        match format {
            I420 => crop_planes(I420_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),

            I422 => crop_planes(I422_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
            I444 => crop_planes(I444_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),

            I010 | I012 => crop_planes(I01X_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
            I210 | I212 => crop_planes(I21X_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
            I410 | I412 => crop_planes(I41X_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),

            NV12 => crop_planes(NV12_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
            YUYV => crop_planes(YUYV_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),

            RGBA | BGRA => crop_planes(RGBA_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
            RGB | BGR => crop_planes(RGB_PLANES, read_planes_mut(planes_mut).unwrap(), self.1),
        }
    }
}
